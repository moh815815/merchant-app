...
def get_products():
    ...
    if result and "results" in result:
        ...
        return pd.DataFrame(rows, columns=["id", "name", "price", "stock", "image_base64"])    return pd.DataFrame()
    ...
    for _, row in df.iterrows():
        with st.container():
            ...
            with col2:
                ...
                st.write(f"Price: {row['price']} EGP | Stock: {row['stock']}")
            st.markdown("---")
else:
